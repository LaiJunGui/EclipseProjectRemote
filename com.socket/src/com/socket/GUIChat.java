...
public void event() {
	...
	private void logFile() throws IOException {
		...
		viewText.setText(str);//不能用append，
		fis.close();
	}
...
public void centerPanel() {
		...
		this.add(center, BorderLayout.CENTER);//将Panel放在Frame的南边
		
	}
public void southPanel() {
		// TODO Auto-generated method stub
		Panel south =new Panel();			//创建南边的Pannel
		tf = new TextField(15);
		...
		south.add(clear);
		south.add(shake);
	   this.add(south,BorderLayout.SOUTH);	//将Panel放在Frame的旁边
	}
private void init() {
	...
	public void run() {
		try {
			...
			while(true) {
						...
						if(arr[0]==-1&&len==1) {//如果发送过来的数组第一个存储的是-1，并且数组的长度是1
							shake();//调用震动方法
